...
def historyAdd(request,id):
    if request.method == 'POST':
        ...
        History.objects.create(title=task.title,desc=task.title,complete=task.complete)
...
def restore(request,id):
    if request.method == 'POST':
        print('inside posttt restorerree')
        task = History.objects.get(id = id)
        Todo.objects.create(title=task.title,desc=task.title,complete=task.complete)
